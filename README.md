...
命令按去除首尾空白后的第一个空白分隔 token 匹配。`/help`、`/status`、`/new` 和
`/stop` 会忽略其余 token；`/model` 和 `/effort` 只把第一个参数作为设置值，后续
token 同样忽略。未知斜杠命令只返回本地提示。命令和多余参数都不会成为 Codex
prompt。
...
命令只由 `text` 消息的文本正文识别；只有首个 token 以 `/` 开头的纯文本消息才是
命令。这样的命令消息即使带有图片引用也只执行命令，不会下载引用图片。`mixed`
...
所有斜杠命令和不支持的消息类型都绕过防抖窗口。`/help`、`/status`、`/model`、
