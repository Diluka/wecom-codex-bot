...
所有纯文本斜杠命令和不支持的消息类型都不产生 request 状态。未知命令在本地直接
返回提示，不会成为 Codex request。
...
命令按去除首尾空白后的空白分隔 token 匹配。通常第一个 token 是命令；企业微信群聊
回调以一个非空 `@mention` token 开头且第二个 token 以 `/` 开头时，第二个 token
是命令。机器人只跳过这一个前缀 token，不在后续正文中搜索命令。
`/help`、`/status`、`/new` 和 `/stop` 会忽略其余 token；`/model` 和 `/effort`
只把命令后的第一个参数作为设置值，后续 token 同样忽略。未知斜杠命令只返回本地
提示。命令和多余参数都不会成为 Codex prompt。
...
默认强度；`/effort <level>` 只接受当前模型支持的强度。第一个参数值非法时，本地
返回可选值或错误原因。
...
命令只由 `text` 消息的文本正文识别；首个 token 以 `/` 开头，或群聊首个 token
是非空 `@mention` 且第二个 token 以 `/` 开头时，才识别为命令。这样的命令消息
即使带有图片引用也只执行命令，不会下载引用图片。单聊中的 mention 前缀和 `mixed`
消息中形似命令的文本按普通用户请求进入防抖窗口。
...
所有斜杠命令和群聊 mention 前缀命令都会立即绕过防抖窗口，不产生 Codex turn。
`/help`、`/status`、`/model`、
`/effort`、未知命令和不支持消息不会重置窗口或中断任务。`/new` 会取消尚未发送的
聚合批次，再按原有会话重置流程创建并绑定新 thread；这个流程不会调用
`turn/start`。`/stop` 会立即清除当前聊天的聚合批次、普通 pending 和待执行的
`/new`，并请求中断活动 turn；它不会删除或替换当前 thread，之后的新普通用户请求
仍可开始新的防抖批次。
