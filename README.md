...
是规范化后的实际 ID；未设置、空值或无效时，`configured` 为
`false`，`owner_user_id` 为 `null`。既有 request 日志仍会按下文规则
记录每条消息真实的 sender `user_id`。
...
启动时的 `owner_configuration` lifecycle 日志会记录规范化后的 owner ID，或者在未
配置、空值、无效时记录 `null`；如果真实请求由该用户发送，上述既有 request
日志也会记录其真实 `user_id`。
