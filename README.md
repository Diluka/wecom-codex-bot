...
目标仓库时，它可以在仓库认可的位置创建隔离 worktree 和新的任务专用非默认分支，
并在其中修改文件、安装依赖、执行仓库规定的验证和提交。首次推送前必须确认远端不
存在同名分支；之后只能推送该任务分支，并在用户要求时为它创建 PR/MR。worktree
位置、分支命名、验证、提交约定和 PR/MR 工作流仍由目标仓库的 `AGENTS.md` 与贡献
文档决定；机器人不固定 Draft 或 Ready。

restricted turn 不得修改 main checkout
的内容、index、stash、未提交内容或无关嵌套
仓库；不得复用或修改任务开始前已经存在的任何远端分支或 PR/MR，也不得提交或推送
默认分支、合并、强推、删除远端引用、发布、部署、变更仓库设置或修改 owner 的全局
配置。它只需解析当前任务的目标仓库；只有任务
