...
- 每次启动都要记录 `owner_configuration` lifecycle 日志；配置有效时输出 owner ID
  的规范化配置值和 `configured: true`，该字段与其他 Pino 字符串一样在超过 100 个
...
- restricted turn 可在 main checkout 中执行无副作用的读取、搜索和状态检查。需要
  修改时，可以在目标仓库认可的位置创建隔离 worktree，并创建或继续非默认任务
  分支；在其中可以修改文件、安装依赖、执行仓库规定的测试、构建和格式化并提交，
  通过非强制推送更新该任务分支，并在用户要求时创建或更新对应的 PR/MR。
...
- restricted turn 不得提交或推送默认分支、强推、删除远端引用、覆盖远端并发更新、
  合并或关闭 PR/MR、发布、部署、变更仓库设置或修改 owner 的全局配置。
